...
Os dados de execução física são, nas metas selecionadas para o IDRGP 2023, desagregadas em nível regional (subprefeitura), podendo, dessa forma, serem extraídos diretamente do SMAE. Para tornar o presente relatório replicável, optou-se por utilizar como fonte sobre a execução física regionalizada os dados abertos do PdM, disponível em: <https://www.prefeitura.sp.gov.br/cidade/secretarias/governo/planejamento/programa_de_metas_20212024/index.php?p=310683>[ ](https://www.prefeitura.sp.gov.br/cidade/secretarias/governo/planejamento/programa_de_metas_20212024/index.php?p=310683)[ ]().
...
-   Regionalização a partir do "Detalhamento da Ação", por meio de base enviada pela Secretaria da Fazenda (SF). Não obstante, a base de dados da regionalização da execução orçamentária do exercício de 2023, pode ser extraída por meio do portal: \<[[https://orcamento.sf.prefeitura.sp.gov.br/orcamente/execucao.php\\\\](https://orcamento.sf.prefeitura.sp.gov.br/orcamente/execucao.php){.uri}](#0){.uri}\>, nos formatos "[.csv](#0)", "[.xlsx](#0)", "[.ods](#0)" ou "[.json](#0)"
...
Introduzidas as passagens que originaram os dados da execução física e orçamentária do Programa de Metas, realizamos, a seguir, as manipulações para a criação de objetos necessários à análise dos dados, a começar pela importação e tratamentos preliminares.
...
1.  Tratar as dotações que possuem codificações incompletas nas bases smae e valor_inf_orgao
2.  Agrupar as dotações, para evitar relações muitos-para-muitos ao combinar bases de dados
3.  Mesclar as bases, separadamente, com a base do detalhamento da ação (visando evitar relações múltiplas entre bases)

A seguir, processamos a base do detalhamento da ação, disponibilizada pela Secretaria da Fazenda.

```{r Base DA SF, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

# obter base de dados do DA

## Base encaminhada pela SF
base_da <- read_xlsx("Base_DA_Limpa_SEPEP.xlsx", sheet = 2) %>%
  janitor::clean_names()
## também poderia ser baixado em "https://orcamento.sf.prefeitura.sp.gov.br/orcamento/uploads/2023/basedadosDA_1223.csv"
## 

glimpse(base_da) #o tipo de algumas colunas pode (dbl em vez de chr) pode afetar a estrutura da dotacao orçamentária (48 caracteres)"

(cod_orgao <- unique(base_da$codigo_orgao)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "10"

(cod_unidade <- unique(base_da$codigo_unidade)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "10"

(cod_funcao <- unique(base_da$codigo_funcao)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "10"

(cod_subfuncao <- unique(base_da$codigo_subfuncao)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "100"

(cod_prog <- unique(base_da$codigo_programa)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "1000"

(cod_proj_ativ <- unique(base_da$codigo_proj_ativ)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "1000"

(cod_conta_desp <- unique(base_da$codigo_conta_despesa)) #ok

(cod_fonte <- unique(base_da$codigo_fonte)) #está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "10"

(cod_exerc <- unique(base_da$codigo_exercicio_fonte)) #ok

(cod_dest <- unique(base_da$codigo_destinacao_recurso)) #ok

(cod_vinc <- unique(base_da$codigo_vinculo_pmsp))#está como dbl. Necessário transformar em chr e acrescentar um "0" a esquerda dos registro menores que "1000"

base_da <- base_da %>%
  mutate(across(c(codigo_orgao,
                  codigo_unidade,
                  codigo_funcao,
                  codigo_subfuncao,
                  codigo_programa,
                  ...
                  codigo_vinculo_pmsp,
                  codigo_tipo_credito_orcamentario), as.character))


# criar coluna com a codificação da dotação orçamentária
base_da <- base_da %>%
  dplyr::mutate(codigo_orgao = ifelse(nchar(codigo_orgao) == 1, str_pad(codigo_orgao, 2, pad = "0"), codigo_orgao),
                codigo_funcao = ifelse(nchar(codigo_funcao) == 1, str_pad(codigo_funcao, 2, pad = "0"), codigo_funcao),
                codigo_subfuncao = ifelse(nchar(codigo_subfuncao) < 3, str_pad(codigo_subfuncao, 3, pad = "0"), codigo_subfuncao),
                codigo_programa = ifelse(nchar(codigo_programa) < 4, str_pad(codigo_programa, 4, pad = "0"), codigo_programa),
                codigo_proj_ativ = ifelse(nchar(codigo_proj_ativ) < 4, str_pad(codigo_proj_ativ, 4, pad = "0"), codigo_proj_ativ),
                codigo_fonte = ifelse(nchar(codigo_fonte) == 1, str_pad(codigo_fonte, 2, pad = "0"), codigo_fonte),
                codigo_vinculo_pmsp = ifelse(nchar(codigo_vinculo_pmsp) < 4, str_pad(codigo_vinculo_pmsp, 4, pad = "0"), codigo_vinculo_pmsp),
                dotacao = base::paste(codigo_orgao,
                                      codigo_unidade,
                                      codigo_funcao,
                                      codigo_subfuncao,
                                      codigo_programa,
...
#checagens
base_da$dotacao[1] #dotacao no formato adequado


base_da %>% 
  select(dotacao, tipo_regionalizacao, valor_detalhamento_acao) %>% 
  filter(str_length(dotacao) < 48) #nenhuma dotação com menos de 48 caracteres
...
# O tratamento das dotações incompletas consiste em ratear o valor total_smae conforme a proporção de cada dotação completa contida no agruamento da dotação incompleta. O produto final será a base_smae com dotacoes equiparáveis à base_da1, mas com os valores total_smae ajustados proporcionalmente.
...
b <- base_inf_orgaos %>% 
  ...
  summarise(meta = first(meta),
            subprefeitura = first(subprefeitura),
            valor_da_inf_orgao = sum(valor_da_inf_orgao))
...
a %>% 
  filter(dotacao == "84.10.10.301.3003.2.520.44505200.00.1.500.7034.1") #como o join entre base_exec_reg_23 e base_da_pdm ampliou o número de linhas, será necessário recalcular a coluna proporcao
...
-   Distribuição territorial dos valores liquidados do PdM

-   Órgãos com maior valor_liquidado em 2023

-   Políticas públicas (ações orçamentárias) com mais recursos liquidados

-   Total liquidado no PdM, considerado no IDRGP 2023 (valor_liquidado)

```{r echo=FALSE, message=FALSE, warning=FALSE}

gp_idrgp_pdm <- base_da_pdm %>% 
  group_by(subprefeitura) %>% 
  summarise(valor_liquidado = sum(valor_liquidado, na.rm = TRUE)) %>% 
...
sum(gp_idrgp_pdm$valor_liquidado)
...
Agora vamos juntar as bases do Orçamento Cidadão, Obras Prioritárias e Programa de Metas para clacular o IDRGP 2023.
...
sum(idrgp$total_idrgp_23)

```

### Intervalo de confiança

Agora, vamos calcular um intervalo de confiança para o percentual liquidado em 2023 e o índice de referência. Para isso, primeiramente, vamos avaliar se os dados correspondem a uma distribuição normal.

Para isso, realizaremos um Teste Kolmogorov-Smirnov.

```{r Testes de normalidade, echo=FALSE}

#Se o p-valor for < 0.05 indica que os dados não apresentam normalidade.

# Teste Kolmogorov-Smirnov para os valores de referência do índice
ks.test(idrgp$ref_idrgp,"pnorm",mean(idrgp$ref_idrgp),sd(idrgp$ref_idrgp))
# O tsste resultou em não rejeição (p-valor > 0.05) da curva normal para os dados. 

#O mesmo teste é repetido para o percentual liquidado de 2023
...
Com isso, torna-se possível construir um intervalo de confiança a partir de um Teste T (*t.test*) pareando a distribuição do índice de referência e o percentual liquidado em 2023.

Para o cálculo deste intervalo de confiança, adotaremos um *t.test pareado*, mantendo o padrão de 95% de confiança, pois as curvas das duas distribuições já estão se ajustando entre si pela diferença.
...
Podemos visualizar a distribuição da distância entre o valor de referênncia e os resultados de 2023 no gráfico abaixo.
...
idrgp %>% 
  ...
  geom_col(aes(x = p_liq_23,
               y = forcats::fct_reorder(NOME, ref_idrgp),
               fill = Status2),
           show.legend = TRUE) +

  scale_fill_manual(values = c("#1A1442","#d97a0f", "#0ea1cf")) +
  ...
  geom_point(mapping = aes(x = ref_idrgp,
             ...
             show.legend = TRUE) +
  scale_colour_manual(values = c("Referência pelo IDRGP" = "#F7C234")) +
...
idrgp %>% 
  ...
  scale_fill_gradient(low="#0ea1cf",high="#1A1442") +
  geom_sf_text(aes(label = sigla), color = "white", size = 2, fontface = "bold") +
  theme_void() +
  labs(title = "Distribuição do valor liquidado calculado no IDRGP 2023",
...
idrgp %>% 
  ...
  scale_fill_gradient(low="#d97a0f",high="#1A1442") +
  geom_sf_text(aes(label = sigla), color = "white", size = 2, fontface = "bold") +
  theme_void() +
...
mapa_22e23 %>% 
  ...
  geom_point(mapping = aes(x = ref_idrgp,
                           ...
                           colour = "Referência pelo IDRGP"),
             show.legend = TRUE) +
  labs(title = "IDRGP no biênio 2022 2023",
       y = "") +
  theme_minimal()+
  theme(legend.title = element_blank(),
        legend.position = "bottom")
...
a <- base_da_pdm %>% 
  group_by(subprefeitura, acao_orcamentaria, orgao) %>% 
...
a <- a %>% 
  mutate(orgao = ifelse(orgao =="SMIT; SETRAM", "SMIT", orgao))
...
b <- df_oc %>% 
  ...
  group_by(subprefeitura, acao_orcamentaria, Secretaria) %>% 
  summarise(liq_oc = sum(liq_oc),
            acao_orcamentaria = first(acao_orcamentaria)) %>% 
  filter(liq_oc > 0)

b <- b %>% 
  mutate(orgao = ifelse(Secretaria =="SVMA/SIURB", "SVMA", Secretaria)) %>% 
  select(subprefeitura, acao_orcamentaria, orgao, liq_oc)

sum(b$liq_oc) == sum(df_oc$Liquidação)

c <- op %>% 
  select(sp_nome, valor_liquidado_2023, acao_orcamentaria, Secretaria) %>% 
  rename(subprefeitura = sp_nome,
  ...
  summarise(liq_op = sum(liq_op),
            acao_orcamentaria = first(acao_orcamentaria)) %>% 
  filter(liq_op > 0) %>% 
  rename(orgao = Secretaria)

sum(c$liq_op) == sum(op$valor_liquidado_2023)

d <-  base_da %>% 
  select(subprefeitura, codigo_proj_ativ, valor_detalhamento_acao) %>% 
  rename(acao_orcamentaria = codigo_proj_ativ) %>% 
  group_by(subprefeitura, acao_orcamentaria) %>% 
  summarise(liq_da = sum(valor_detalhamento_acao, na.rm = TRUE),
            acao_orcamentaria = first(acao_orcamentaria)) %>% 
  mutate(acao_orcamentaria = str_replace(acao_orcamentaria, "^(.{1})(.*)$", "\\1.\\2"))

e <- d %>% 
  ungroup() %>% 
  group_by(subprefeitura) %>% 
  select(-acao_orcamentaria) %>% 
  summarise(liq_da_sub = sum(liq_da)) %>% 
  filter(!is.na(liq_da_sub))

sum(e$liq_da_sub) == sum(base_da$valor_detalhamento_acao, na.rm = TRUE)
...
fichas_subp <- fichas_subp %>% 
  left_join(d, by = intersect(names(fichas_subp), names(d))) %>% 
  replace_na((list(liq_pdm = 0, liq_oc = 0, liq_op = 0, liq_da = 0)))

fichas_subp <- fichas_subp %>% 
  left_join(e, by = "subprefeitura") %>% 
  replace_na((list(liq_pdm = 0, liq_oc = 0, liq_op = 0, liq_da = 0, liq_da_sub = 0)))
...
fichas_subp <- fichas_subp %>% 
  ...
  select(subprefeitura, acao_orcamentaria, descricao_proj_ativ, orgao, liq_da_sub, liq_23, liq_pdm, liq_oc, liq_op, liq_da)

sum(fichas_subp$liq_23)

ref_idrgp.x <- ref_idrgp %>% 
  mutate(posicao_ranking = seq(1:32)) %>% 
  rename(subprefeitura = sp_nome)

fichas_subp <- fichas_subp %>% 
  left_join(ref_idrgp.x, by = "subprefeitura")

colnames(mapa_22e23)

mapa_22e23.x <- mapa_22e23 %>% 
  select(sp_nome, idrgp_2022, total_idrgp_23, liq_22e23, Status) %>% 
  rename(subprefeitura = sp_nome,
         liq_idrgp_22 = idrgp_2022,
         liq_idrgp_23 = total_idrgp_23,
         liq_idrbp_acum = liq_22e23,
         status = Status)

fichas_subp <- fichas_subp %>% 
  left_join(mapa_22e23.x, by = "subprefeitura")

fichas_subp %>% 
  filter(acao_orcamentaria == "4.432")

fichas_subp <- fichas_subp %>% 
  mutate(descricao_proj_ativ = if_else(acao_orcamentaria == "4.432", "Capacitação, Formação e Aperfeiçoamento dos Trabalhadores", descricao_proj_ativ))

colnames(fichas_subp)

fichas_subp <- fichas_subp %>% 
  select(subprefeitura, liq_da_sub, ref_idrgp, posicao_ranking, liq_idrgp_22, liq_idrgp_23, liq_idrbp_acum, status, acao_orcamentaria, descricao_proj_ativ,orgao, liq_23, liq_da, liq_pdm, liq_oc, liq_op) %>% 
  arrange(subprefeitura, desc(liq_23))

readr::write_excel_csv2(fichas_subp, "fichas_subp.csv")

sum(fichas_subp$liq_pdm)
sum(fichas_subp$liq_oc)
sum(fichas_subp$liq_op)
sum(fichas_subp$liq_23)

fichas_subp %>% 
  group_by(subprefeitura) %>% 
  summarise(sum(liq_23))
...
```{r graficos para a apresentação, echo=FALSE, message=FALSE, warning=FALSE}
#visualizações básicas
fichas_subp %>% 
  group_by(orgao) %>% 
  summarise(Liquidado = sum(liq_23)) %>% 
  filter(Liquidado >= 15000000) %>% 
  ggplot() +
  geom_col(aes(x = Liquidado,
               y = reorder(orgao, Liquidado)), 
           fill = "#d97a0f",
           show.legend = TRUE) +
  labs(title = "Os 10 órgãos com maior participação no IDRGP",
       x = NULL, y = NULL) +
  theme_minimal()
...
fichas_subp %>% 
   ...
   summarise(Liquidado = sum(liq_23)) %>%
   filter(Liquidado >= 30000000) %>% 
   ggplot() +
   geom_col(aes(x = Liquidado,
                y = reorder(descricao_proj_ativ, Liquidado)),
            fill = "#d97a0f",
            show.legend = TRUE) +
   labs(title = "Os 10 maiores gastos por Ação Orçamentária", 
        subtitle = "Liquidado IDRGP (2023)",
         x = NULL, y = NULL)+
   theme_minimal()
