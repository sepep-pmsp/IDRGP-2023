...
total_smae_totais_metas <- sum(base_pdm$total_smae) #3116974852 Necessário ajustar dotações incompletas
...
(caso3_base_b <- base_da_pdm %>% 
  filter(str_detect(dotacao, "^14\\.10\\.16\\.482\\.3005\\.3\\.355\\.44905100\\.00")))

colnames(caso3_base_b)

(caso3_merge <- tibble(orgao = rep(caso3_base_b$sigla_orgao, 7),
...
x <- base_pdm %>% 
  select(meta, dotacao, orgao, total_smae) %>% 
  group_by(meta, dotacao, orgao) %>% 
  summarise(total_smae = sum(total_smae))

sum(x$total_smae) == total_smae_totais_metas
...
url_balanco <- "https://www.prefeitura.sp.gov.br/cidade/secretarias/upload/governo/planejamento/Relat%C3%B3rio%20de%20Execu%C3%A7%C3%A3o%20Anual%20do%20Programa%20de%20Metas%202021-2024%20(Abril2024).xlsx" #note que o documento é de abril. Caso o documento seja atualizado, necessário alter o link
