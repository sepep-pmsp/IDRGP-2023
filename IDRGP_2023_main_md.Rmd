...
colnames(suporte_rateio_regioes)
...
teste2 <- comp_sub_dot %>% 
  left_join(teste, by = c("dotacao", "subprefeitura"))
...
teste2 %>% 
  left_join(c, by = c("dotacao", "sigla_orgao", "regiao", "distrito", "tipo_regionalizacao", "valor_detalhamento_acao"))

#juntar com sigla_orgao, distrito e tipo_regionalizacao
(a <- base_da1 %>% 
    select(c("dotacao", "sigla_orgao", "tipo_regionalizacao")))

teste2 %>% 
  left_join(a, by = c("dotacao")) %>% 
  ...
  select(dotacao, sigla_orgao, subprefeitura, regiao, tipo_regionalizacao, valor_detalhamento_acao, count, valor_rateio)
...
(b <- base_da1 %>% 
    group_by(dotacao) %>% 
    select(-subprefeitura, -regiao, -distrito, -valor_detalhamento_acao))
...
(base_da2 <- a %>% 
  mutate(sigla_orgao = if_else(is.na(sigla_orgao.x), sigla_orgao.y, sigla_orgao.x),
    ...
    TRUE ~ "Sul"),
         tipo_regionalizacao = if_else(is.na(tipo_regionalizacao.x), tipo_regionalizacao.y, tipo_regionalizacao.x))%>%
      select(dotacao, sigla_orgao, subprefeitura, regiao, distrito, tipo_regionalizacao, valor_detalhamento_acao, -distrito, -sigla_orgao.x, -distrito.x, -tipo_regionalizacao.x, -sigla_orgao.y, -distrito.y, -tipo_regionalizacao.y,
           ))
...
(teste2 %>% 
  filter(valor_rateio > 0))


suporte_rateio_regioes %>% 
