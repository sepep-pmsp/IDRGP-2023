...
# base_da <- readr::read_csv2("basedadosDA_1223.csv",
...
## Está dando erro aqui! linha 847
...
#### Gráfico ação orçamentária

```{r}

a <- subprefeitura %>% 
  as_tibble() %>% 
  select(sp_nome, sigla) %>% 
  rename(subprefeitura = sp_nome)

b <- fichas_subp %>% 
  left_join(a, by = "subprefeitura")

b %>%
  ungroup() %>%
  group_by(status) %>% 
  filter(status == "Acima da referência do IDRGP" & liq_23 > 1000000) %>% 
  arrange(desc(liq_23), subprefeitura) %>% 
  ggplot() +
  geom_col(mapping = aes(x = fct_reorder(sigla, liq_23, .desc = TRUE), y = liq_23)) +
  facet_wrap(vars(descricao_proj_ativ)) +
...
b %>%
  ungroup() %>%
  group_by(status) %>%
  filter(status == "Aquem da referência do IDRGP" & liq_23 > 1000000) %>%
  arrange(desc(liq_23), subprefeitura) %>%
  ggplot() +
  geom_col(mapping = aes(x = fct_reorder(sigla, liq_23, .desc = TRUE), y = liq_23)) +
  geom_text(aes(x = sigla, y = liq_23, label = sigla), vjust = -0.5, size = 2.5) +
  facet_wrap(vars(descricao_proj_ativ)) +
  scale_y_continuous(labels = scales::comma_format(big.mark = ".", decimal.mark = ",")) +
  labs(
    title = "Valores liquidado 2023 - Subprefeituras aquém da referência do IDRGP",
    x = NULL,
    y = NULL
  ) +
...
b %>%
  ungroup() %>%
  group_by(status) %>%
  filter(status == "Dentro da referência" & liq_23 > 1000000) %>%
  arrange(desc(liq_23), subprefeitura) %>%
  ggplot() +
  geom_col(mapping = aes(x = fct_reorder(sigla, liq_23, .desc = TRUE), y = liq_23)) +
  geom_text(aes(x = sigla, y = liq_23, label = sigla), vjust = -0.5, size = 2.5) +
  ...
  scale_y_continuous(labels = scales::comma_format(big.mark = ".", decimal.mark = ",")) +
  labs(
    title = "Valores liquidado 2023 - Subprefeituras dentro da referência do IDRGP",
    x = NULL,
    y = NULL
  ) +
  theme(axis.text.y = element_text(size = 8),
...
b %>%
  ...
  scale_y_continuous(labels = scales::comma_format(big.mark = ".", decimal.mark = ",")) +
  labs(
    title = "Valores liquidado 2023 - Subprefeituras acima da referência do IDRGP",
    x = NULL,
    y = NULL
  ) +
  theme(axis.text.y = element_text(size = 8),
...
b %>%
  ...
  filter(status == "Acima da referência do IDRGP" & liq_23 > 1000000) %>%
  arrange(desc(liq_23), subprefeitura) %>%
  mutate(descricao_proj_ativ_wrapped = str_wrap(descricao_proj_ativ, width = 30)) %>%  # Adiciona quebra de linha
  ggplot() +
  geom_col(mapping = aes(x = fct_reorder(sigla, liq_23, .desc = TRUE), y = liq_23)) +
  geom_text(aes(x = sigla, y = liq_23, label = sigla), vjust = -0.5, size = 2.5) +
  facet_wrap(vars(descricao_proj_ativ_wrapped)) +
  scale_y_continuous(labels = scales::comma_format(big.mark = ".", decimal.mark = ",")) +
  labs(
    title = "Valores liquidado 2023 - Subprefeituras acima da referência do IDRGP",
    x = NULL,
...
sum(base_da1$valor_detalhamento_acao)

b %>% 
  group_by(descricao_proj_ativ) %>% 
  summarise(liq_23 = sum(liq_23)) %>% 
  arrange((desc(liq_23)))

base_da %>% 
  filter(codigo_proj_ativ == 2386)

```

### Outros gráficos

* Gráfico de Órgão por Subprefeitura

```{r graficos para a apresentação, echo=FALSE}
...
#Graficos por órgão
ggplot(fichas_subp, aes(y = subprefeitura, x = liq_23)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_line(data = fichas_subp , aes(y = subprefeitura, x = liq_23), color = "orange", linetype = "dashed", linewidth = 0.25) +
  facet_wrap(vars(orgao), nrow = 6, ncol = 6)+
  labs(x = NULL, y = NULL, fill = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),
...
#Graficos por sub
ggplot(fichas_subp, aes(x = orgao, y = liq_23)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_line(data = fichas_subp , aes(x = orgao, y = liq_23), color = "orange", linetype = "dashed", linewidth = 0.25) +
  facet_wrap(vars(subprefeitura), nrow = 6, ncol = 6)+
  labs(x = NULL, y = NULL, fill = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),
...
# orçamento sub
da_sub<- base_da1 %>% mutate(e_sub = str_extract(sigla_orgao,"SUB-")) %>% group_by(e_sub) %>%  summarise(sum(valor_detalhamento_acao))

```

Varificando o vlalor total

```{r}
Investiemnto_total<- base_da %>% 
  filter(tipo_regionalizacao=="Despesa Regionalizável") 
Investiemnto_total<- Investiemnto_total%>% 
  mutate(manter = str_sub(codigo_conta_despesa, start = 1, end = 1)) %>% 
  filter(manter==4)
...
sum(Investiemnto_total$valor_detalhamento_acao)

invest_tot<- Investiemnto_total %>% 
  group_by(subprefeitura) %>% 
  summarise(total_liq= sum(valor_detalhamento_acao))
invest_tot<- invest_tot %>% 
  rename(sp_nome=subprefeitura)

invest_tot<- subprefeitura %>% 
  inner_join(invest_tot, by= "sp_nome")


invest_tot<- invest_tot %>% 
  mutate(p_liq_23 = total_liq/ sum(total_liq),
        diferenca = p_liq_23 - ref_idrgp) %>% 
  select(sigla, NOME, sp_nome, geometry, ref_idrgp, total_liq, p_liq_23, diferenca)

teste_t_invest_tot<-t.test(invest_tot$ref_idrgp, invest_tot$p_liq_23, paired = T)
invest_tot<- invest_tot %>% 
  mutate("Status" = case_when(
    invest_tot$diferenca <  teste_t_invest_tot$conf.int[1] ~ "Aquem da referência do IDRGP",
    invest_tot$diferenca >  teste_t_invest_tot$conf.int[2] ~ "Acima da referência do IDRGP",
    TRUE ~ "Dentro da referência"))

# Mapa 1 - valor liquidado 2023
invest_tot %>% 
  ggplot() +
  geom_sf(aes(fill = total_liq, geometry = geometry)) +
  scale_fill_gradient(low = "#0ea1cf", high = "#1A1442", labels = label_number(big.mark = ".", decimal.mark = ",")) +
  geom_sf_text(aes(label = sigla), color = "white", size = 2, fontface = "bold") +
  theme_void() +
  labs(
    title = "Distribuição do investimento em 2023",
    caption = "Elaboração: SGM/SEPEP/CP",
    x = NULL, y = NULL
  )
...
invest_tot %>% 
  ...
  geom_sf_text(aes(label = sigla, geometry = geometry), color = "white", size = 2, fontface = "bold") +
  theme_void() +
  labs(title = "Se o IDRGP fosse o investimento total no periodo",
       subtitle = "t.test com intervalo de confiança de 95%",
       fill = "Aproximação da referência do IDRGP",
       caption = "Elaboração: SGM/SEPEP/CP.",
       x = NULL, y = NULL)
