...
# ajustar nas bases df_pdm1 e df_pdm2 observações cujas dotações não estão com a codificação completa

teste3 <- df_pdm %>% 
  mutate(dotacao_completa = if_else(nchar(DOTACAO) == 48, "S", "N"))

teste3 %>% 
  filter(dotacao_completa == "N")

base_dotacao_pdm <- unique(teste3$DOTACAO)

(teste1 <- df_pdm1 %>%
  dplyr::filter(!base::nchar(DOTACAO)==48 &
                  valor_da > 0))
...
teste2 <- df_pdm2 %>% 
  dplyr::filter(!base::nchar(DOTACAO)==48 &
                  valor_da_inf_orgao > 0)

sum(teste2$valor_da_inf_orgao)

sum(teste2$valor_da_inf_orgao)+sum(teste1$valor_da)
