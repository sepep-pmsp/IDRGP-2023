...
a <- base_da1
  #filter(!str_detect(subprefeitura, "Supra")) #remover as linhas que contêm "supra" para diminuir o tamanho do df comp_sub_dot
...
# Checar se todas as dotações possuem contegem 36 para subprefeitura (32 subs + 4 supra)
(a <- comp_sub_dot %>% 
...
checagem6 <- sum(a$valor_detalhamento_acao) == soma_da_base2
...
b <- a %>% 
  mutate(total_smae = total_smae*proporcao)

sum(b$total_smae) == soma_smae

soma_smae - sum(b$total_smae) # diferença de R$ 18,5 milhões
...
c <- b %>% 
  group_by(meta) %>% 
  summarise(chec_da = sum(total_smae))
...
e <- c %>% 
  ...
  filter(diferenca != 0) # a diferença se dá exclusivamente na meta 4
...
sum(k$chec_da)
sum(k$chec_da_pdm) - soma_base_da_pdm # Embora não tenha sido encontrada diferença no objeto k, a comparação com soma_base_da_pdm apresenta diferença de R$ 580,9 mil. Não sendo possível identificar a origem dessa diferença, vamos desconsiderar esse valor
