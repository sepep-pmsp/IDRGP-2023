...
fichas_subp %>% 
  ...
  geom_col(aes(x = Liquidado,
               y = orgao),
           show.legend = TRUE) +
  labs(title = "Liquidado IDRGP por Órgãos") +
  theme_minimal()
...
fichas_subp %>% 
  ...
  ggplot() +
  geom_col(aes(x = Liquidado,
               y = descricao_proj_ativ),
           show.legend = TRUE) +
  labs(title = "Liquidado IDRGP por Ação Orçamentária") +
  theme_minimal()
...
for (i in unique(fichas_subp$orgao)) {nome_grafico = paste(i,”.png”, sep = “”)png(filename = nome_grafico, width = 15, height = 15, units = “cm”, pointsize = 12, res = 150)
print(ggplot(fichas_subp, aes(y = subprefeitura, x = liq_23)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_line(data = fichas_subp , aes(y = subprefeitura, x = liq_23), color = "orange", linetype = "dashed", linewidth = 0.25) +
  facet_wrap(vars(orgao), nrow = 6, ncol = 6)+
  labs(x = NULL, y = NULL, fill = NULL) +
...
fichas_subp %>% group_by(orgao) %>% 
    ggplot() +
    geom_bar(aes(x = subprefeitura, y = liq_23)) +
  labs(x = NULL, y = NULL, fill = NULL) +
  theme_minimal() 

ggplot(fichas_subp, aes(x = subprefeitura, y = liq_23, fill = orgao)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_line(data = fichas_subp , aes(x = subprefeitura, y = liq_23, group = orgao), color = "orange", linetype = "dashed", linewidth = 0.25) +
  facet_wrap(vars(orgao), nrow = 3)+
  labs(x = NULL, y = NULL, fill = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),
