...
op <- op %>% 
  ...
  mutate(acao_orcamentaria = str_replace(`Cod projeto`,"^(.{1})(.*)$",  "\\1.\\2"),
         id= `...1`) %>%
  select(id, acao_orcamentaria,  Secretaria, Subprefeitura, valor_liquidado_2023)
...
df_oc <- df_oc1 %>% 
  filter(IDRGP == "Sim") %>% 
  mutate(acao_orcamentaria = DOTACAO) %>% 
  select(`Nº Proposta`, acao_orcamentaria, Secretaria, Subprefeitura, Liquidação)
df_oc<- df_oc %>% 
...
df_oc <- df_oc %>% 
  mutate(acao_orc = stringr::str_sub(acao_orcamentaria, start = 19, end = 23))
