...
#1 14.10.16.451.3002.3.357.44903900.00
(a <- base_da2 %>% 
  filter(str_detect(dotacao, "^14\\.10\\.16\\.451\\.3002\\.3\\.357\\.44903900\\.00") & valor_liquidado > 0) %>% 
  group_by(dotacao) %>% 
  summarise(valor_liquidado = sum(valor_liquidado), .groups = "drop") %>% 
  mutate(total_valor = sum(valor_liquidado),
         prop = valor_liquidado / total_valor)) # para identificar quantas dotações são abragidas pela dotação incompleta calcular a proporção de cada dotação completa no conjunto abrangido pela dotação incompleta
...
#2 14.10.16.451.3005.3.355.44903900.00
(a <- base_da2 %>% 
  filter(str_detect(dotacao, "^14\\.10\\.16\\.451\\.3005\\.3\\.355\\.44903900\\.00") & valor_liquidado > 0) %>% 
  group_by(dotacao) %>% 
  summarise(valor_liquidado = sum(valor_liquidado), .groups = "drop") %>% 
  mutate(total_valor = sum(valor_liquidado),
         prop = valor_liquidado / total_valor)) # para identificar quantas dotações são abragidas pela dotação incompleta calcular a proporção de cada dotação completa no conjunto abrangido pela dotação incompleta
...
# neste caso, há sete dotações; então vamos substituir para cada uma as informações que constam nas dotacoes incompletas, criando um df provisório (c) que deverá ter 32x7 = 224 linhas
b <- base_da_pdm %>% 
...
# neste caso, há duas dotações; então 
b <- base_da_pdm %>% 
